...
class adxl343:
    ...
    def get_acceleration(self):
        ''' Return acceleration in all three axes (X, Y, Z) as a tuple '''
        x = self.read_x_axis()
        y = self.read_y_axis()
        z = self.read_z_axis()
        return (x, y, z)
...
if __name__ == "__main__":
    sensor = adxl343()
    while 1:
        x_acceleration, y_acceleration, z_acceleration = sensor.get_acceleration()
        print(f"X: {x_acceleration}, Y: {y_acceleration}, Z: {z_acceleration}")
        time.sleep(1)
